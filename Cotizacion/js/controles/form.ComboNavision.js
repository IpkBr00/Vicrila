...
ComboNavision.prototype.setValor = function(valor){
    ...
    if(this.estaCargado)
        ...
        setTimeout( function(){ self.setValor(valor); }, 1000);
};
...
ComboNavision.prototype.ordenarDatos = function(datos){
    var self = this;
    var datosOrdenados = [];
    if(this.campo.IdReferencia == 1)
    {
        _.each(datos, function(registro){ return registro[self.fuente.CampoValor + '_orderByField'] = normalize(registro[self.fuente.CampoValor]); });
        datosOrdenados= _.sortBy(datos, function(registro){ return normalize(registro[self.fuente.CampoValor + '_orderByField']); });
    }
    else
    {
