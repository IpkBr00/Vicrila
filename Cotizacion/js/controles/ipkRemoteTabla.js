        ...
        if(respuesta.estado = 'OK')
        {
            ...
            else
            {
                if(self.datosPasados)
                    ...
                    if(self.propiedades.datosPadre){
                        var entidad = self.propiedades.datosPadre.Entidad;
                        var clave = self.propiedades.datosPadre.Clave;
                        var valor = self.propiedades.datosPadre.Valor;
                        self.tabla.setDatos(_.filter(respuesta.datos, function(p){return p[entidad][clave] == valor;}));
                    }
                    else
                    {
                        self.tabla.setDatos(respuesta.datos);
...
IpkRemoteTabla.prototype.obtenerDatosListado = function(){
    var clave = _.find(this.modelo.zz_CamposModelos, function(elemento) { return elemento.EsClave == true;});
    var buscar = {};
    buscar[clave.Nombre] = '0 OR True = True';

    if(this.propiedades.esME)
        this.genericoDS.Buscar(buscar, true, true);
    else
