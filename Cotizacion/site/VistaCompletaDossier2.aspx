<html lang="es">
	...
	<body>
        ...
        <script type="text/javascript">
            ...
            i.onGetModelos = function(listadoModelos){
                console.log('Listado de Modelos');
                console.log('*************************');
                console.log(listadoModelos);
                for(var j in listadoModelos)
                {
                    i.getModeloByName(listadoModelos[j].Nombre);
                    i.getListadoByName(listadoModelos[j].Nombre);
                    i.getFichaByName(listadoModelos[j].Nombre);
                }
            };

            i.onGetFuenteInternas = function(listado){
                console.log('Listado de Fuentes Internas');
                console.log('*************************');
                console.log(listado);
                for(var j in listado)
                {
                    i.getFuenteInternaByName(listado[j].Nombre);
                }
            };


            i.onGetModelo = function(modelo){
            ...
            i.onGetListado = function(listado){
                console.log('Get de Listado');
                console.log('*************************');
                console.log(listado.Nombre);
                console.log(listado);
                listados[listado.Nombre] = listado;
            };

            i.onGetFicha = function(ficha){
            ...
            i.onGetFuenteInterna = function(fuente){
                console.log('Get de fuente');
                console.log('*************************');
                console.log(fuente.Nombre);
                console.log(fuente);
                fuentes[fuente.Nombre] = fuente;
            };
            ...
            $(document).ready(function(){
                ...
                objInspector.render();
                 ...
                 {
                     solucion = new ObjectInspector( $soluciones, soluciones[i], { titulo : 'SOLUCION' , clase: 'colPeterRiver', colapsable: true});
                     inspectores.Solucion.push(solucion);
                     solucion.render();

                     for(var j = 0; j < soluciones[i].FormaArt.length; j++)
                         new ObjectInspector($articulos, soluciones[i].FormaArt[j], { titulo : 'ARTICULO' , clase: 'colCarrot', colapsable: true}).render();

                     for(var k = 0; k < soluciones[i].Nivel.length; k++)
                     {
                         new ObjectInspector($embalajes, soluciones[i].Nivel[k], { titulo : 'NIVEL' , clase: 'colAmethyst', colapsable: true}).render();
                         for(var l = 0; l < soluciones[i].Nivel[k].FormaEmb.length; l++)
                             new ObjectInspector($embalajes, soluciones[i].Nivel[k].FormaEmb[l], { titulo : 'EMBALAJE' , clase: 'colAlizarin', colapsable: true, nivel : 1}).render();
                     }
                 }
